...
int CMyApp::Program_Begin(HINSTANCE	hInstance, int nCmdShow)
{
    ...
    while (TRUE)
    {
		...
		m_MeshManager.Clear_BackBuffer();

		//Angle = PI / 2.75f;

		//сначала рисуем тень потом куб
		
		//draw shadow
		m_MeshManager.Update_MeshManager(true, Angle);
		m_MeshManager.Draw_MeshManager(true);

		//draw cube
		m_MeshManager.Update_MeshManager(false, Angle);
		m_MeshManager.Draw_MeshManager(false);

		m_MeshManager.Present_BackBuffer();
