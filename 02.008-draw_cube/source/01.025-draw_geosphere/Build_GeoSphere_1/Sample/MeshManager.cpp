...
void CMeshManager::Init_MeshManager(HWND hWnd)
{
	...
	unsigned int m_TriangleCountStart = 60 / 3;

	m_TriangleCount = 20;
	m_VertCount = 12;

	m_TriangleCount = m_TriangleCount * (int) pow(4.0f, (int) NumSubdivisions);
	m_VertCount = m_TriangleCount * 6;

	m_VertBuff = new vector3[m_VertCount];
	vector3 * m_VertBuffTemp = new vector3[m_VertCount];
	m_VertBuffTransformed = new vector3[m_VertCount];
	m_IndexBuff = new unsigned int[m_TriangleCount * 3];
	unsigned int * m_IndexBuffTemp = new unsigned int[m_TriangleCount * 3];

	memset(m_VertBuff,0, sizeof(vector3) * m_VertCount);
	memset(m_VertBuffTemp,0, sizeof(vector3) * m_VertCount);
	memset(m_IndexBuff,0, sizeof(unsigned int) * m_TriangleCount * 3);
	memset(m_IndexBuffTemp,0, sizeof(unsigned int) * m_TriangleCount * 3);

	memcpy(m_VertBuff, Pos, sizeof(vector3) * 12);
	memcpy(m_IndexBuff, Indx, sizeof(unsigned int) * 60);
	...
	{
		memcpy(m_VertBuffTemp, m_VertBuff, sizeof(vector3) * m_VertCount);
		memcpy(m_IndexBuffTemp, m_IndexBuff, sizeof(unsigned int) * m_TriangleCount * 3);

		int Index1 = 0;
		int Index2 = 0;

		for(unsigned int i = 0; i < m_TriangleCountStart; i++)
		{
			...
			m_VertBuff[i * 6 + 0] = v0; // 0
			m_VertBuff[i * 6 + 1] = v1; // 1
			m_VertBuff[i * 6 + 2] = v2; // 2
			m_VertBuff[i * 6 + 3] = m0; // 3
			m_VertBuff[i * 6 + 4] = m1; // 4
			m_VertBuff[i * 6 + 5] = m2; // 5

			Index1 += 6;

			m_IndexBuff[i * 12 + 0] = i*6+0;
			m_IndexBuff[i * 12 + 1] = i*6+3;
			m_IndexBuff[i * 12 + 2] = i*6+5;

			m_IndexBuff[i * 12 + 3] = i*6+3;
			m_IndexBuff[i * 12 + 4] = i*6+4;
			m_IndexBuff[i * 12 + 5] = i*6+5;

			m_IndexBuff[i * 12 + 6] = i*6+5;
			m_IndexBuff[i * 12 + 7] = i*6+4;
			m_IndexBuff[i * 12 + 8] = i*6+2;

			m_IndexBuff[i * 12 + 9] = i*6+3;
			m_IndexBuff[i * 12 + 10] = i*6+1;
			m_IndexBuff[i * 12 + 11] = i*6+4;

			Index2 += 4;
		}
		m_TriangleCountStart = Index2;
	}
