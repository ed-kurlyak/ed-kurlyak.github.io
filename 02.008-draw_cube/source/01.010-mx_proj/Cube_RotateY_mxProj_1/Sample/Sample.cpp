...
vector4 g_VertBuffTransformed[8];
...
vector4 Vec3_Mat4x4_Mul_H(vector3 &VecIn, matrix4x4 MatIn)
{
	vector4 VecOut;
...
void Draw_Cube()
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		//Vec.w = Vec.z после умножения
		//вектора на матрицу проекции
		
		vector4 VecH = Vec3_Mat4x4_Mul_H(Vec, MatProj);

		VecH.x = VecH.x / VecH.w;	//перспективное деление
		VecH.y = VecH.y / VecH.w;	//перспективное деление
		VecH.z = VecH.z / VecH.w;	//это значение нужно в DirectX API для Z буфера
									//мы это значение не используем
		VecH.w = 1.0f / VecH.w;	//это значение нужно в DirectX API для
								//правильнгого отображения текстуры
								//с учетом перcпективы
								//мы это значение не используем

		//преобразование в экранные координаты
		VecH.x = VecH.x * Rc.right / 2.0f + Rc.right / 2.0f;
		VecH.y = -VecH.y * Rc.bottom / 2.0f + Rc.bottom / 2.0f;

		g_VertBuffTransformed[i] = VecH;
	}
	...
	for (int i = 0; i < 12; i++)
	{
		vector4 Vec1 = g_VertBuffTransformed[ g_IndexBuff[ i * 2 + 0 ] ];
		vector4 Vec2 = g_VertBuffTransformed[ g_IndexBuff[ i * 2 + 1 ] ];

		//Vec.z и Vec.w не используются
