...
int m_AtanBaseTable[8] = {
    0x0000, -0x4000, -0xFFFF, 0xC000, -0x8000, 0x4000, 0x8000, -0xC000,
};

unsigned short m_AtanAngleTable[0x802] = {
...
{
    unsigned short int sector = (unsigned short int)Angle & (PHD_180 - 1);
    if (sector > PHD_90) {
        sector = PHD_180 - sector;
    }
    short int result = m_SinTable[sector >> 4];
    if ((unsigned short int)Angle >= PHD_180) {
        result = -result;
    }
    return result;
}
...
int Math_Atan(int x, int y)
{
    ...
    int result = m_AtanBaseTable[base] + m_AtanAngleTable[0x800 * y / x];
    if (result < 0) {
        result = -result;
    }
...
unsigned int Math_Sqrt(unsigned int n)
{
    unsigned int result = 0;
    unsigned int base = 0x40000000;
    do {
        do {
            unsigned int based_result = base + result;
            result >>= 1;
            if (based_result > n) {
                break;
            }
