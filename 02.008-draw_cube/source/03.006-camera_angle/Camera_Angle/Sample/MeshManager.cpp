...
void CMeshManager::Get_View_Matrix()
{
	...
	{
		m_VecCamPos.x += -sinf(m_CamAngle) * Time * RatioMove; 
		m_VecCamPos.z += cosf(m_CamAngle) * Time * RatioMove;
	}
	...
	{
		m_VecCamPos.x += -cosf(m_CamAngle) * Time * RatioMove;
		m_VecCamPos.z += -sinf(m_CamAngle) * Time * RatioMove;
	}
...
void CMeshManager::Update_MeshManager()
{
	...
	for ( int j =0; j < m_Polygons.PolygonCount; j++)
	{
		...
		Vec1.z = dz1 * cosf(m_CamAngle) - dx1 * sinf(m_CamAngle);
		...
		if (Verts)
		{
			for ( int i = 1; i < Verts - 1; i++)
			{
				...
				Poly.Vertex[Index++] = ClippedVertices[0];
				Poly.Vertex[Index++] = ClippedVertices[i];
				Poly.Vertex[Index++] = ClippedVertices[i + 1];
		...
		for ( int i = 0; i < FrontList.PolygonCount; i++)
		{
			...
			Vec1.x = Vec1.x * m_ViewHeight / 2.0f + m_ViewWidth / 2.0f;
			Vec1.y = -Vec1.y * m_ViewHeight / 2.0f + m_ViewHeight / 2.0f;

			Vec2.x = Vec2.x * m_ViewHeight / 2.0f + m_ViewWidth / 2.0f;
			Vec2.y = -Vec2.y * m_ViewHeight / 2.0f + m_ViewHeight / 2.0f;

			Vec3.x = Vec3.x * m_ViewHeight / 2.0f + m_ViewWidth / 2.0f;
			...
			if(Verts)
			{
				for ( int j = 1; j < Verts - 1; j++ )
				{
					...
					pt1.Vertex[Index++] = Vertex[0];
					pt1.Vertex[Index++] = Vertex[j];
					pt1.Vertex[Index++] = Vertex[j+1];
