...
void Draw_Cube()
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		//так же в расчет можно добавить проекцию 1/tanf()
		Vec.x = Vec.x * Rc.bottom / 2.0f * (1.0f / tanf(Fov * 0.5f)) / Vec.z + Rc.right / 2.0f;
		Vec.y =-Vec.y * Rc.bottom / 2.0f * (1.0f / tanf(Fov * 0.5f)) / Vec.z + Rc.bottom / 2.0f;
