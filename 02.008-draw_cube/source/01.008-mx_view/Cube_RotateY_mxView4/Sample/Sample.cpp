...
vector3 Vec3_Mat4x4_View_Mul(vector3 &VecIn, matrix4x4 MatIn)
{
	vector3 VecOut;
	...
	//умножаем на VecCamRight
	VecOut.x =	VecIn.x * MatIn[0][0] +
				VecIn.y * MatIn[1][0] +
				VecIn.z * MatIn[2][0];

	//умножаем на VecCamUp
	VecOut.y =  VecIn.x * MatIn[0][1] +
				VecIn.y * MatIn[1][1] +
				VecIn.z * MatIn[2][1];
					      
	//умножаем на VecCamLook
	VecOut.z =  VecIn.x * MatIn[0][2] +
				VecIn.y * MatIn[1][2] +
				VecIn.z * MatIn[2][2];
...
void Draw_Cube()
{
	...
	for ( int i = 0; i < 8; i++ )
	{
		...
		Vec = Vec3_Mat4x4_View_Mul(Vec, MatView);
